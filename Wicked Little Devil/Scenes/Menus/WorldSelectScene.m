...
- (void) tap_facebook
{
    ...
    if ([PFUser currentUser] && [PFFacebookUtils isLinkedWithUser:[PFUser currentUser]])
    {
        ...
        [[CCDirector sharedDirector] pushScene:[CCTransitionMoveInB transitionWithDuration:1.0f scene:[GameOverFacebookScene sceneWithGame:tmpgame fromScene:2]]];
    }
    else
    {
...
- (void) tap_world:(CCMenuItemFont*)sender
{
    ...
    [[CCDirector sharedDirector] replaceScene:[CCTransitionCrossFade transitionWithDuration:0.2f scene:[LevelSelectScene sceneWithWorld:sender.tag]]];
}
